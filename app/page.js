...
export default function Home() {
  ...
  const [displayMessage, setDisplayMessage] = useState("Za warudo!");
  ...
  return (
    <main className="container mx-auto max-w-4xl">
      <div className="grid grid-cols-2">
        <div
          className={`bg-yellow-500 relative py-4 px-4 flex flex-col justify-center ${
            loading ? "animate-pulse" : ""
          }`}
        >
          <div className="absolute h-[15px] w-[15px] bg-yellow-500 -right-[7px] top-[50%] rotate-45"></div>
          <h3 className="text-2xl text-white bold">Dio Says:</h3>
          <p className="text-white">
            {loading ? "[Dio is thinking]" : parse(marked(displayMessage))}
          </p>
        </div>

        <div>
          <Image alt="Botty" src="/diobrando.png" width={312} height={212} />
        </div>
      </div>
      ...
      <form className="mt-6" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-4">
          ...
          <input
            ...
            type="text"
            ref={messageRef}
            placeholder="Don't be afraid, ask him a question."
          />
        </div>
